...
public class LocationController {
    ...
    public ResponseEntity<List<LocationDto>> getAllLocations(){
        ...
        return ResponseEntity.ok(locationList.stream()
                .map(dtoMapper::toDto)
                .toList());
    }
    @PostMapping()
    public ResponseEntity<LocationDto> createLocation(
      ...
      var createdLocation= locationService.createLocation(dtoMapper.toDomain(locationDto));
        return ResponseEntity.status(201).body(dtoMapper.toDto(createdLocation));
    }
           ...
           @PathVariable("locationId") Long locationId)
    {
        ...
        return ResponseEntity.ok(dtoMapper.toDto(foundLocation));
    }
    ...
    public ResponseEntity<LocationDto> updateLocation(
        ...
        var updatedLocation=locationService.updateLocation(dtoMapper.toDomain(updateLocationDto),locationId);
