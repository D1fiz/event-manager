...
public class LocationService {
    ...
    public LocationService(LocationRepository repository, LocationEntityMapper entityMapper) {
    ...
    public Location updateLocation(Location locationToUpdate, Long locationId) {
        ...
        var entityToUpdate = repository.findById(locationId).orElseThrow(() -> new EntityNotFoundException("Location entity was not found id=%s".formatted(locationId)));
    ...
    public Location deleteLocation(Long locationId) {
        var entityToDelete = repository.findById(locationId).orElseThrow(() -> new EntityNotFoundException("Location entity was not found id=%s".formatted(locationId)));
        repository.deleteById(locationId);
        return entityMapper.toDomain(entityToDelete);
    }

    public Location getLocationById(Long locationId) {
        var foundEntity = repository.findById(locationId).orElseThrow(() -> new EntityNotFoundException("Location entity was not found id=%s".formatted(locationId)));
